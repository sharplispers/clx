...
(defun create-window (&key
  ...
  (let* ((display (drawable-display parent))
	 (window (or window (make-instance 'window :display display)))
...
(defun destroy-window (window)
  ...
  (let ((display (drawable-display window)))
    (with-buffer-request (display +x-destroywindow+)
      (window window))
    (deallocate-resource-id display (drawable-id window) 'window)))

(defun destroy-subwindows (window)
  (declare (type window window))
  (with-buffer-request ((drawable-display window) +x-destroysubwindows+)
    (window window)))

(defun add-to-save-set (window)
  (declare (type window window))
  (with-buffer-request ((drawable-display window) +x-changesaveset+)
    (data 0)
    (window window)))
...
(defun reparent-window (window parent x y)
  (declare (type window window parent)
	   (type int16 x y))
  (with-buffer-request ((drawable-display window) +x-reparentwindow+)
    (window window parent)
    (int16 x y)))

(defun map-window (window)
  (declare (type window window))
  (with-buffer-request ((drawable-display window) +x-mapwindow+)
    (window window)))

(defun map-subwindows (window)
  (declare (type window window))
  (with-buffer-request ((drawable-display window) +x-mapsubwindows+)
    (window window)))

(defun unmap-window (window)
  (declare (type window window))
  (with-buffer-request ((drawable-display window) +x-unmapwindow+)
    (window window)))

(defun unmap-subwindows (window)
  (declare (type window window))
  (with-buffer-request ((drawable-display window) +x-unmapsubwindows+)
    (window window)))

(defun circulate-window-up (window)
  (declare (type window window))
  (with-buffer-request ((drawable-display window) +x-circulatewindow+)
    (data 0)
    (window window)))
...
(defun convert-selection (selection type requestor &optional property time)
  (declare (type xatom selection type)
	   ...
	   (type timestamp time))
  (let* ((display (drawable-display requestor))
...
(defun send-event (window event-key event-mask &rest args
  ...
  (unless display (setq display (drawable-display window)))
...
(defun grab-button (window button event-mask
  ...
  (declare (type window window)
	   ...
	   (type (or null window) confine-to)
	   (type (or null cursor) cursor))
  (with-buffer-request ((drawable-display window) +x-grabbutton+)
    ((data boolean) owner-p)
    (window window)
    (card16 (encode-pointer-event-mask event-mask))
    (boolean (not sync-pointer-p) (not sync-keyboard-p))
...
(defun ungrab-button (window button &key (modifiers :any))
  (declare (type window window)
	   (type (or (member :any) card8) button)
	   (type modifier-mask modifiers))
  (with-buffer-request ((drawable-display window) +x-ungrabbutton+)
...
(defun grab-key (window key &key (modifiers 0) owner-p sync-pointer-p sync-keyboard-p)
  (declare (type window window)
	   ...
	   (type modifier-mask modifiers))
  (with-buffer-request ((drawable-display window) +x-grabkey+)
    ((data boolean) owner-p)
    (window window)
    (card16 (encode-modifier-mask modifiers))
...
(defun ungrab-key (window key &key (modifiers 0))
  (declare (type window window)
	   (type (or (member :any) card8) key)
	   (type modifier-mask modifiers))
  (with-buffer-request ((drawable-display window) +x-ungrabkey+)
...
(defun translate-coordinates (src src-x src-y dst)
  ...
  (let ((display (drawable-display src)))
...
(defun warp-pointer (dst dst-x dst-y)
  ...
  (with-buffer-request ((drawable-display dst) +x-warppointer+)
    (resource-id 0) ;; None
    (window dst)
    (int16 0 0)
    (card16 0 0)
...
(defun warp-pointer-relative-if-inside (x-off y-off src src-x src-y
  ...
  (declare (type window src)
	   ...
	   (type (or null card16) src-width src-height))
  (unless (or (eql src-width 0) (eql src-height 0))
    (with-buffer-request ((drawable-display src) +x-warppointer+)
      (window src)
      (resource-id 0) ;; None
      (int16 src-x src-y)
      (card16 (or src-width 0) (or src-height 0))
...
(defun create-pixmap (&key
  ...
  (let* ((display (drawable-display drawable))
	 (pixmap (or pixmap (make-instance 'pixmap :display display)))
	 (pid (allocate-resource-id display pixmap 'pixmap)))
    (setf (drawable-id pixmap) pid)
...
(defun free-pixmap (pixmap)
  ...
  (let ((display (drawable-display pixmap)))
    (with-buffer-request (display +x-freepixmap+)
      (pixmap pixmap))
    (deallocate-resource-id display (drawable-id pixmap) 'pixmap)))
...
(defun clear-area (window &key (x 0) (y 0) width height exposures-p)
  ...
  (unless (or (eql width 0) (eql height 0))
    (with-buffer-request ((drawable-display window) +x-cleartobackground+)
...
(defun create-colormap (visual-info window &optional alloc-p)
  ...
  (let ((display (drawable-display window)))
    ...
    (let* ((colormap (make-instance 'colormap :display display :visual-info visual-info))
...
(defun copy-colormap-and-free (colormap)
  ...
  (declare (clx-values colormap))
  (let* ((display (colormap-display colormap))
	 (new-colormap (make-instance 'colormap :display display
...
(defun create-cursor (&key
  ...
  (let* ((display (drawable-display source))
	 (cursor (make-instance 'cursor :display display))
